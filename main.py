...
# Increase body size limit for base64 photos (50MB)
...
async def send_to_group(data: dict):
    ...
    if amount:
        lines.append("")
        lines.append("Сумма ущерба: " + amount + " " + currency)
    if victims:
        lines.append("Кол-во жертв: " + str(victims) + " чел.")
    lines += [
        "",
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if photo and "base64" in photo:
            try:
                ...
                r = await client.post(tg + "/sendMessage",
...
async def get_user_by_username(username: str):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id, name, username, photo, role FROM users WHERE LOWER(username)=? OR LOWER(username)=?",
              (username, "@"+username))
    row = c.fetchone()
    conn.close()
    if not row:
        return {"exists": False}
    role = "admin" if row[0] in ADMIN_IDS else ("mod" if row[0] in MOD_IDS else row[4])
    return {"exists": True, "tg_id": row[0], "name": row[1], "username": row[2], "photo": row[3], "role": role}
...
def init_blacklist_db():
    ...
    c.execute(
        ...
        "id INTEGER PRIMARY KEY, name TEXT, username TEXT, letter TEXT, photo TEXT,"
        "threat TEXT DEFAULT 'high', status TEXT DEFAULT 'active', victims INTEGER DEFAULT 1,"
        "reason TEXT, amount TEXT, currency TEXT DEFAULT 'USDT',"
        "date_str TEXT, ts INTEGER, proofs TEXT DEFAULT '[]', added_by INTEGER)"
    )
    conn.commit()
    conn.close()
...
async def get_blacklist():
    ...
    for row in rows:
        entry = {
            "id": row[0], "name": row[1], "username": row[2], "letter": row[3],
            "photo": row[4], "threat": row[5], "status": row[6], "victims": row[7],
            "reason": row[8], "amount": row[9], "currency": row[10],
            "date": row[11], "ts": row[12]
        }
        try: entry["proofs"] = json.loads(row[13] or "[]")
        except: entry["proofs"] = []
        result.append(entry)
...
async def add_to_blacklist(entry: BlacklistEntry):
    ...
    c.execute(
        "INSERT INTO blacklist (name,username,letter,photo,threat,status,victims,reason,amount,currency,date_str,ts,proofs,added_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
...
async def update_blacklist_entry(entry_id: int, data: dict):
    ...
    c.execute("UPDATE blacklist SET " + ",".join(fields) + " WHERE id=?", values)
    conn.commit()
    conn.close()
    return {"ok": True}
...
async def get_all_users():
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id, name, username FROM users")
    rows = c.fetchall()
    conn.close()
    return {"ok": True, "users": [{"tg_id": r[0], "name": r[1], "username": r[2]} for r in rows]}

@app.post("/notify-users")
async def notify_users(data: dict):
    """Trigger notification to all users via bot"""
    scammer = data.get("scammer", {})
    if not scammer:
        return {"ok": False, "error": "scammer data required"}
    # Call bot's notify function via Telegram API directly
    try:
        conn = get_db()
        c = conn.cursor()
        ...
        if amount:
            lines.append("")
            lines.append("Ущерб: " + amount + " " + currency)
