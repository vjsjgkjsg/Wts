...
async def send_to_group(data: dict):
    ...
    msg = (
        f"🚨 *НОВЫЙ СКАМЕР В БАЗЕ WTS*
"
        f"{'─'*28}

"
        f"👤 *{scammer.get('name','?')}* {scammer.get('username','')}
"
        f"⚠️ Статус: {threat_map.get(scammer.get('threat','high'), '🔴 СКАМЕР')}
"
        f"🔒 Аккаунт: {status_map.get(scammer.get('status','active'), '🔴 Активен')}

"
        f"📋 *Причина:*
_{scammer.get('reason','')}_
"
    )
    if scammer.get("amount"):
        msg += f"
💸 *Ущерб:* {scammer.get('amount')} {scammer.get('currency','USDT')}"
    if scammer.get("victims"):
        msg += f"
👥 *Жертв:* {scammer.get('victims')}"
    msg += f"

🛡 _WTS Blacklist — Защита сообщества_"
