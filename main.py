...
async def add_to_blacklist(entry: BlacklistEntry):
    ...
    # Extract TG ID from username via lookup — non-blocking, 5s timeout
    tg_id_val = None
    try:
        import asyncio
        entity = await asyncio.wait_for(client.get_entity(entry.username), timeout=5.0)
        tg_id_val = entity.id
    except Exception:
        pass  # Never block the save if Telethon fails
