import os
import httpx
import hashlib
import json
import time
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel
from telethon import TelegramClient
import psycopg2
from psycopg2.extras import RealDictCursor

app = FastAPI()
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

BOT_TOKEN   = os.getenv("BOT_TOKEN", "8274782796:AAFBK4sJpQhtXnIE9IxOMmNhivlM2dXEgp4")
API_ID      = int(os.getenv("API_ID", "26508724"))
API_HASH    = os.getenv("API_HASH", "2ada38c67ea946fe3be7fdd8e2507366")
DATABASE_URL = os.getenv("DATABASE_URL", "")

def parse_ids(env_var):
    val = os.getenv(env_var, "").strip()
    if not val:
        return []
    try:
        return [int(x.strip()) for x in val.split(",") if x.strip().isdigit()]
    except:
        return []

ADMIN_IDS = parse_ids("ADMIN_IDS")
MOD_IDS   = parse_ids("MOD_IDS")
print(f"ADMIN_IDS loaded: {ADMIN_IDS}")

TG = f"https://api.telegram.org/bot{BOT_TOKEN}"

def get_db():
    return psycopg2.connect(DATABASE_URL, cursor_factory=RealDictCursor)

def init_db():
    conn = get_db()
    c = conn.cursor()
    c.execute("""
        CREATE TABLE IF NOT EXISTS users (
            tg_id     BIGINT PRIMARY KEY,
            name      TEXT,
            username  TEXT,
            photo     TEXT,
            role      TEXT DEFAULT 'user',
            pin_hash  TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            last_login TIMESTAMP
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS groups (
            chat_id   BIGINT PRIMARY KEY,
            title     TEXT,
            added_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS blacklist (
            id        SERIAL PRIMARY KEY,
            name      TEXT,
            username  TEXT,
            letter    TEXT,
            photo     TEXT,
            threat    TEXT DEFAULT 'high',
            status    TEXT DEFAULT 'active',
            victims   INTEGER DEFAULT 1,
            reason    TEXT,
            amount    TEXT,
            currency  TEXT DEFAULT 'USDT',
            date_str  TEXT,
            ts        BIGINT,
            proofs    TEXT DEFAULT '[]',
            added_by  BIGINT
        )
    """)
    conn.commit()
    conn.close()

client = TelegramClient("wts_session", API_ID, API_HASH)

@app.on_event("startup")
async def startup():
    init_db()
    await client.start(bot_token=BOT_TOKEN)
    print("Server started!")

@app.on_event("shutdown")
async def shutdown():
    await client.disconnect()

def hash_pin(pin):
    return hashlib.sha256(pin.encode()).hexdigest()

class RegisterRequest(BaseModel):
    tg_id:    int
    name:     str
    username: str
    photo:    str = None
    pin:      str

class LoginRequest(BaseModel):
    tg_id: int
    pin:   str

class ChangePinRequest(BaseModel):
    tg_id:   int
    old_pin: str
    new_pin: str

class BlacklistEntry(BaseModel):
    name:     str
    username: str
    letter:   str = "?"
    photo:    str = None
    threat:   str = "high"
    status:   str = "active"
    victims:  int = 1
    reason:   str
    amount:   str = ""
    currency: str = "USDT"
    date_str: str = ""
    ts:       int = 0
    proofs:   list = []
    added_by: int = None

class GroupRequest(BaseModel):
    chat_id: int
    title:   str = "Группа"

class GroupRemove(BaseModel):
    chat_id: int

# ── USER ROUTES ──

@app.get("/user/{tg_id}")
async def get_user(tg_id: int):
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id, name, username, photo, role FROM users WHERE tg_id=%s", (tg_id,))
    row = c.fetchone()
    conn.close()
    if not row:
        return {"exists": False}
    role = "admin" if tg_id in ADMIN_IDS else ("mod" if tg_id in MOD_IDS else row["role"])
    return {"exists": True, "tg_id": row["tg_id"], "name": row["name"], "username": row["username"], "photo": row["photo"], "role": role}

@app.get("/user/by-username/{username}")
async def get_user_by_username(username: str):
    username = username.lstrip("@").strip().lower()
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id, name, username, photo, role FROM users WHERE LOWER(username)=%s OR LOWER(username)=%s", (username, "@"+username))
    row = c.fetchone()
    conn.close()
    if not row:
        return {"exists": False}
    tg_id = row["tg_id"]
    role = "admin" if tg_id in ADMIN_IDS else ("mod" if tg_id in MOD_IDS else row["role"])
    return {"exists": True, "tg_id": tg_id, "name": row["name"], "username": row["username"], "photo": row["photo"], "role": role}

@app.post("/register")
async def register(req: RegisterRequest):
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id FROM users WHERE tg_id=%s", (req.tg_id,))
    if c.fetchone():
        conn.close()
        return {"ok": False, "error": "Пользователь уже зарегистрирован"}
    role = "admin" if req.tg_id in ADMIN_IDS else ("mod" if req.tg_id in MOD_IDS else "user")
    c.execute("INSERT INTO users (tg_id, name, username, photo, role, pin_hash) VALUES (%s,%s,%s,%s,%s,%s)",
        (req.tg_id, req.name, req.username, req.photo, role, hash_pin(req.pin)))
    conn.commit()
    conn.close()
    return {"ok": True, "role": role}

@app.post("/login")
async def login(req: LoginRequest):
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id, name, username, photo, role, pin_hash FROM users WHERE tg_id=%s", (req.tg_id,))
    row = c.fetchone()
    if not row:
        conn.close()
        return {"ok": False, "error": "Пользователь не найден"}
    if row["pin_hash"] != hash_pin(req.pin):
        conn.close()
        return {"ok": False, "error": "Неверный PIN"}
    c.execute("UPDATE users SET last_login=CURRENT_TIMESTAMP WHERE tg_id=%s", (req.tg_id,))
    conn.commit()
    conn.close()
    tg_id = row["tg_id"]
    role = "admin" if tg_id in ADMIN_IDS else ("mod" if tg_id in MOD_IDS else row["role"])
    return {"ok": True, "tg_id": tg_id, "name": row["name"], "username": row["username"], "photo": row["photo"], "role": role}

@app.post("/change-pin")
async def change_pin(req: ChangePinRequest):
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT pin_hash FROM users WHERE tg_id=%s", (req.tg_id,))
    row = c.fetchone()
    if not row:
        conn.close()
        return {"ok": False, "error": "Пользователь не найден"}
    if row["pin_hash"] != hash_pin(req.old_pin):
        conn.close()
        return {"ok": False, "error": "Неверный текущий PIN"}
    c.execute("UPDATE users SET pin_hash=%s WHERE tg_id=%s", (hash_pin(req.new_pin), req.tg_id))
    conn.commit()
    conn.close()
    return {"ok": True}

@app.get("/update-role")
async def update_role(tg_id: int, role: str = None):
    conn = get_db()
    c = conn.cursor()
    if not role:
        role = "admin" if tg_id in ADMIN_IDS else ("mod" if tg_id in MOD_IDS else "user")
    if role not in ["admin", "mod", "user"]:
        conn.close()
        return {"ok": False, "error": "Invalid role"}
    c.execute("UPDATE users SET role=%s WHERE tg_id=%s", (role, tg_id))
    conn.commit()
    conn.close()
    return {"ok": True, "role": role, "admin_ids": ADMIN_IDS}

@app.get("/users/all")
async def get_all_users():
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id, name, username, role FROM users")
    rows = c.fetchall()
    conn.close()
    return {"ok": True, "users": [dict(r) for r in rows]}

# ── BLACKLIST ──

@app.get("/blacklist")
async def get_blacklist():
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT id,name,username,letter,photo,threat,status,victims,reason,amount,currency,date_str,ts,proofs FROM blacklist ORDER BY ts DESC")
    rows = c.fetchall()
    conn.close()
    result = []
    for row in rows:
        entry = dict(row)
        entry["date"] = entry.pop("date_str", "")
        try:
            entry["proofs"] = json.loads(entry.get("proofs") or "[]")
        except:
            entry["proofs"] = []
        result.append(entry)
    return {"ok": True, "blacklist": result}

@app.post("/blacklist/add")
async def add_to_blacklist(entry: BlacklistEntry):
    conn = get_db()
    c = conn.cursor()
    ts = entry.ts or int(time.time()*1000)
    c.execute("""INSERT INTO blacklist (name,username,letter,photo,threat,status,victims,reason,amount,currency,date_str,ts,proofs,added_by)
                 VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
        (entry.name, entry.username, entry.letter, entry.photo, entry.threat, entry.status,
         entry.victims, entry.reason, entry.amount, entry.currency, entry.date_str, ts,
         json.dumps(entry.proofs), entry.added_by))
    new_id = c.fetchone()["id"]
    conn.commit()
    conn.close()
    return {"ok": True, "id": new_id}

@app.delete("/blacklist/{entry_id}")
async def delete_from_blacklist(entry_id: int):
    conn = get_db()
    c = conn.cursor()
    c.execute("DELETE FROM blacklist WHERE id=%s", (entry_id,))
    conn.commit()
    conn.close()
    return {"ok": True}

@app.put("/blacklist/{entry_id}")
async def update_blacklist_entry(entry_id: int, data: dict):
    conn = get_db()
    c = conn.cursor()
    allowed = ["reason","amount","threat","status","victims"]
    fields = [f"{k}=%s" for k in data if k in allowed]
    values = [v for k,v in data.items() if k in allowed]
    if not fields:
        conn.close()
        return {"ok": False, "error": "No valid fields"}
    values.append(entry_id)
    c.execute("UPDATE blacklist SET " + ",".join(fields) + " WHERE id=%s", values)
    conn.commit()
    conn.close()
    return {"ok": True}

# ── GROUPS ──

@app.post("/groups/add")
async def add_group(req: GroupRequest):
    conn = get_db()
    c = conn.cursor()
    c.execute("INSERT INTO groups (chat_id, title) VALUES (%s,%s) ON CONFLICT (chat_id) DO UPDATE SET title=%s",
              (req.chat_id, req.title, req.title))
    conn.commit()
    conn.close()
    return {"ok": True}

@app.post("/groups/remove")
async def remove_group(req: GroupRemove):
    conn = get_db()
    c = conn.cursor()
    c.execute("DELETE FROM groups WHERE chat_id=%s", (req.chat_id,))
    conn.commit()
    conn.close()
    return {"ok": True}

@app.get("/groups")
async def get_groups():
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT chat_id, title FROM groups ORDER BY added_at DESC")
    rows = c.fetchall()
    conn.close()
    return {"ok": True, "groups": [dict(r) for r in rows]}

# ── SEND TO GROUP ──

@app.post("/send-to-group")
async def send_to_group(data: dict):
    import base64 as b64
    chat_id = data.get("chat_id")
    scammer = data.get("scammer", {})
    if not chat_id:
        return {"ok": False, "error": "chat_id required"}

    tmap = {"high": "СКАМЕР", "med": "ПОДОЗРИТЕЛЬНЫЙ", "low": "ОСТОРОЖНО"}
    smap = {"active": "Активен - мошенничает", "blocked": "Заблокирован в Telegram"}

    name     = scammer.get("name", "Неизвестный")
    username = scammer.get("username", "")
    threat   = tmap.get(scammer.get("threat", "high"), "СКАМЕР")
    status   = smap.get(scammer.get("status", "active"), "Активен")
    reason   = scammer.get("reason", "")
    amount   = str(scammer.get("amount", ""))
    currency = scammer.get("currency", "USDT")
    victims  = scammer.get("victims", 0)
    photo    = scammer.get("photo", None)
    proofs   = scammer.get("proofs", [])
    date     = scammer.get("date", "")

    lines = ["ВНИМАНИЕ - СКАМЕР ОБНАРУЖЕН", ""]
    lines.append("Имя: " + name + ("  |  " + username if username else ""))
    lines.append("Категория: " + threat)
    lines.append("Статус: " + status)
    if date:
        lines.append("Дата: " + date)
    lines += ["", "Причина:", reason]
    if amount:
        lines += ["", "Ущерб: " + amount + " " + currency]
    if victims:
        lines.append("Жертв: " + str(victims) + " чел.")
    lines += ["", "WTS Blacklist - защита вашего сообщества"]
    msg_text = "\n".join(lines)

    tg_url = "https://api.telegram.org/bot" + BOT_TOKEN

    async with httpx.AsyncClient(timeout=30) as client:
        if photo and "base64" in photo:
            try:
                b64data = photo.split(",", 1)[1]
                img_bytes = b64.b64decode(b64data)
                r = await client.post(tg_url + "/sendPhoto",
                    data={"chat_id": str(chat_id), "caption": msg_text},
                    files={"photo": ("photo.jpg", img_bytes, "image/jpeg")})
            except Exception:
                r = await client.post(tg_url + "/sendMessage",
                    json={"chat_id": chat_id, "text": msg_text})
        else:
            r = await client.post(tg_url + "/sendMessage",
                json={"chat_id": chat_id, "text": msg_text})

        if not r.json().get("ok"):
            return {"ok": False, "error": r.json().get("description", "Ошибка")}

        valid_proofs = [p for p in proofs if p and "base64" in p][:10]
        if valid_proofs:
            await client.post(tg_url + "/sendMessage",
                json={"chat_id": chat_id, "text": "Доказательства (" + str(len(valid_proofs)) + " скрин.):"})
            for idx, proof in enumerate(valid_proofs):
                try:
                    img_bytes = b64.b64decode(proof.split(",", 1)[1])
                    await client.post(tg_url + "/sendPhoto",
                        data={"chat_id": str(chat_id), "caption": "Скрин " + str(idx+1)},
                        files={"photo": ("proof.jpg", img_bytes, "image/jpeg")})
                except Exception as e:
                    print("Proof error:", e)

    return {"ok": True}

# ── NOTIFY ──

@app.post("/notify-users")
async def notify_users(data: dict):
    scammer = data.get("scammer", {})
    if not scammer:
        return {"ok": False, "error": "scammer data required"}
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT tg_id FROM users")
    rows = c.fetchall()
    conn.close()

    name     = scammer.get("name", "Неизвестный")
    username = scammer.get("username", "")
    threat   = scammer.get("threat", "high")
    reason   = scammer.get("reason", "")
    amount   = str(scammer.get("amount", ""))
    currency = scammer.get("currency", "USDT")
    victims  = scammer.get("victims", 0)

    emoji = "🔴" if threat == "high" else ("🟠" if threat == "med" else "🔵")
    label = "СКАМЕР" if threat == "high" else ("ПОДОЗРИТЕЛЬНЫЙ" if threat == "med" else "ОСТОРОЖНО")

    lines = [emoji + " НОВЫЙ СКАМЕР В БАЗЕ WTS", ""]
    lines.append("Имя: " + name + ("  |  " + username if username else ""))
    lines.append("Категория: " + label)
    lines.append("", reason)
    if amount:
        lines += ["", "Ущерб: " + amount + " " + currency]
    if victims:
        lines.append("Жертв: " + str(victims) + " чел.")
    lines += ["", "WTS Blacklist - будь осторожен при сделках"]
    msg = "\n".join(lines)

    sent = 0
    async with httpx.AsyncClient(timeout=5) as client:
        for row in rows:
            try:
                await client.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                    json={"chat_id": row["tg_id"], "text": msg})
                sent += 1
            except Exception:
                continue
    return {"ok": True, "sent": sent, "total": len(rows)}

# ── LOOKUP ──

@app.get("/lookup")
async def lookup(username: str):
    username = username.lstrip("@").strip()
    if not username:
        return {"ok": False, "error": "Введи username"}
    try:
        entity = await client.get_entity(f"@{username}")
        first = getattr(entity, "first_name", "") or ""
        last  = getattr(entity, "last_name", "") or ""
        uname = getattr(entity, "username", username) or username
        full_name = f"{first} {last}".strip() or uname
        user_id = entity.id
        photo_b64 = None
        try:
            import base64
            photo_bytes = await client.download_profile_photo(entity, file=bytes)
            if photo_bytes:
                photo_b64 = "data:image/jpeg;base64," + base64.b64encode(photo_bytes).decode()
        except Exception:
            pass
        return {"ok": True, "id": user_id, "name": full_name, "username": f"@{uname}", "photo": photo_b64, "letter": full_name[0].upper() if full_name else "?"}
    except Exception as e:
        err = str(e)
        if "Cannot find" in err or "No user" in err:
            return {"ok": True, "id": None, "name": username, "username": f"@{username}", "photo": None, "letter": username[0].upper(), "note": "private"}
        return {"ok": False, "error": "Пользователь не найден"}

@app.get("/health")
async def health():
    return {"status": "ok", "admin_ids": ADMIN_IDS}
