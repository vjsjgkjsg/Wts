...
class BlacklistEntry(BaseModel):
    id: int = None  # ignored, server generates via BIGSERIAL
    ...
    date_str: str = ""; date: str = ""; ts: int = 0; proofs: list = []; added_by: int = None
    tg_id: int = None  # optional, filled from lookup
...
async def add_to_blacklist(entry: BlacklistEntry):
    ...
    date_str = entry.date_str or getattr(entry, "date", "") or ""
    # Try to resolve TG ID — non-blocking, with timeout so it never hangs
    tg_id_val = None
    try:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            entry.victims, entry.reason, entry.amount, entry.currency, date_str, ts,
